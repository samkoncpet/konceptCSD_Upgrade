...
export class NewcustomersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.addcustomer = this._formBuilder.group({
      fatherfirstrname: new FormControl('', [
        Validators.required,
        AlphaValidator,
      ...
      fathercellno: new FormControl('', [
        NumericValidator,
        Validators.minLength(10),
        Validators.maxLength(15),
      ]),
      mothercellno: new FormControl('', [
        NumericValidator,
        Validators.minLength(10),
        Validators.maxLength(15),
      ]),
    ...
    this.getPaymentMode();
    this.subscriptionStartMinDate = this.pipe
      .transform(this.currentDate, 'yyyy-MM-dd')
      .toString();
    this.subscriptionEndMinDate = this.pipe
      .transform(this.currentDate, 'yyyy-MM-dd')
      .toString();
  }
  ...
  getCustomerDetail() {
    ...
    this._ConfigurationService.post(url, data).subscribe(
      (response) => {
        if (response['response'] == 1) {
          ...
          this.addcustomer.patchValue({
            ...
            is_active: this._commonfunctionsService.getBoolean(
      ...
      CustomerInfoUpdateList: [
        {
          ...
          mportant_Notes: '',
          Is_Active: this._commonfunctionsService.getBoolean(
            this.addcustomer.value.is_active
          ),
        },
      ],
    };
  ...
  addstudentlist() {
    if (this.studentlist.length <= 5) {
      if (this.param1 == 'update' || this.updateStudentID > 0) {
        if (
          (this.addstudent.value.studentfirstname == '' ||
            this.addstudent.value.studentfirstname == undefined) &&
          (this.addstudent.value.studentlastname == '' ||
            this.addstudent.value.studentlastname == undefined) &&
          (this.addstudent.value.gradeid == '' ||
            this.addstudent.value.gradeid == undefined)
        ) {
          this.submitted = true;
          ...
          if (data != null) {
            ...
            this.studentlist[index].FirstName =
              this.addstudent.value.studentfirstname;
            this.studentlist[index].LastName =
              this.addstudent.value.studentlastname;
      ...
      this._notificationsService.info(
        'Cannot add more than 5 students per customer!',
        'info'
      );
    }
  }
  ...
  deleteStudent(id: number) {
    var Customer_Child_ID = this.studentlist.find(
      (x) => x.id == id
    ).Customer_Child_ID;
  ...
  onPasswordChange() {
    if (
      this.addcustomer.get('password').value ==
      this.addcustomer.get('repassword').value
    ) {
      ...
      (response) => {
        if (response['response'] == 1) {
          ...
          this.addpackage.patchValue({
            subscriptiondate: this.pipe
              .transform(response['data'][0].Start_Date, 'yyyy-MM-dd')
              .toString(),
            subscriptionenddate: this.pipe
              .transform(response['data'][0].Cancellation_Date, 'yyyy-MM-dd')
              .toString(),
      ...
      (response) => {
        if (response['response'] == 1) {
          ...
          for (var i = 0; i < response['data'].length; i++) {
            ...
            stuObj.Customer_Child_ID = parseInt(
              response['data'][i].Customer_Child_ID
            );
  ...
  updateSubscriptionDetails() {
    ...
    this.studentlist = [];
    var url = this._appSettings.koncentAPI;
    var updatecustomersubscriptionAPI =
      this._appSettings.updatecustomersubscriptionAPI;
      ...
      CustomerSubscriptionUpdateList: [
        {
          ...
          Is_Active: this._commonfunctionsService.getBoolean(
            this.addpackage.value.is_active
          ),
        },
      ],
    };

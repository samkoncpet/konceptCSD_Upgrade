import { Component, OnDestroy } from '@angular/core';
import { NgAceToasterService } from 'ng-ace-admin';

@Component({
  selector: 'app-toast-examples',
  templateUrl: './toast-examples.component.html'
})
export class ToastExamplesComponent implements OnDestroy {

  constructor(private toasterService: NgAceToasterService) { }

  ngOnDestroy() {
    // remove all toasts instantly when this component is destroyed (NavigationStart)
    this.toasterService.removeAll(null, true)
  }

  showDefault() {
    this.toasterService.show({
      placement: 'tc',
      className: 'bgc-white-tp1 border-none border-t-4 brc-primary-tp1 rounded-sm pl-3 pr-1',

      headerClass: 'bg-transparent border-0 text-120 text-blue-d3 font-bolder mt-3 pl-5 ml-4',
      bodyClass: 'd-flex pt-0 pb-3 px-1 text-105',
      closeClass: 'btn btn-light-danger border-0 btn-bgc-tp btn-xs px-2 py-0 text-150 position-tr mt-n25',

      header: 'Notification message!',
      body: `
            <i class="fas fa-exclamation-circle fa-3x text-blue mt-n4 ml-1 mr-35"></i>

            <p class="mb-0">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
            </p>
      `
    }).then((result: any) => {
      // console.log(result)
    })
  }


  showInfo() {
    this.toasterService.show({
      placement: 'tr',
      className: 'bgc-white-tp1 shadow ',

      headerClass: 'd-none',
      bodyClass: 'border-0 p-0 text-dark-tp2',

      body: `<p class='p-3 mb-0 text-center'>
              <span class='d-inline-block text-center mb-3 py-3 px-1 border-1 brc-success radius-round'>
                <i class='fa fa-check fa-2x w-6 text-success-m1 mx-2px'></i>
              </span><br />
            
              Lorem ipsum dolor sit amet, consectetur adipiscing elit!
            </p>

            <button class='close-btn btn btn-block btn-info radius-t-0 border-0'>
              OK
            </button>`
    })
  }


  showWarning() {
    this.toasterService.show({
      placement: 'tr',
      className: 'bgc-white-tp1 shadow border-0',

      headerClass: 'd-none',
      bodyClass: 'd-flex border-0 p-0 text-dark-tp2',

      body: `
            <div class='bgc-orange-d1 text-white px-3 pt-3'>
                <div class='border-2 brc-white px-3 py-25 radius-round'>
                    <i class='fa fa-times text-150'></i>
                </div>
            </div>
            <div class='p-3 mb-0 flex-grow-1'>
                <h4 class='text-130'>Warning</h4>
                Lorem ipsum dolor sit amet!
            </div>
            <button class='close-btn align-self-start btn btn-xs btn-outline-grey btn-h-light-grey py-2px mr-1 mt-1 border-0 text-150'>&times;</button>
          `
    })
  }

  showSuccess() {
    this.toasterService.show({
      placement: 'rc',

      width: 360,
      delay: 4000,
    
      className: 'bgc-green-d2 shadow',

      headerClass: 'd-none',
      bodyClass: 'border-0 p-0',

      body: `
          <p class='p-3 mb-0 text-center text-white'>
            <span class='d-inline-block mb-3'>
              <i class='fa fa-check fa-2x'></i>
            </span><br />

            <span class='text-125'>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit!
            </span>
          </p>
          `
        ,
        progress: 'position-bl bgc-black-tp6 py-2px m-1px'
    })
  }


  showSuccess2() {
    this.toasterService.show({
      placement: 'rc',

      width: 360,
      delay: 4000,
    
      className: 'bgc-success-d2 shadow ',

      headerClass: 'd-none',
      bodyClass: 'border-0 p-0',

      body: `
        <p class='p-3 mb-0 text-center text-white'>
            <span class='d-inline-block mb-3 border-2 bgc-white radius-round p-25'>
                <i class='fa fa-check fa-2x mx-1px text-success'></i>
            </span><br />

            <span class='text-125'>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit!
            </span>
        </p>
        
        <button class='close-btn close position-tr mt-1 mr-2 text-white'>&times;</button>
        `
        ,
        progress: 'position-bl bgc-white-tp4 py-2px m-1px',
        progressReverse: true
    })
  }


  showError() {
    this.toasterService.show({
      placement: 'br',

      width: 480,
    
      className: 'shadow border-none radius-0 border-l-4 brc-danger',

      headerClass: 'd-none',
      bodyClass: 'border-0 p-0',

      body: `
        <div class='p-3 m-2 d-flex'>
          <span class='align-self-center text-center mr-3 py-2 px-1 border-1 bgc-danger radius-round'>
            <i class='fa fa-times text-180 w-4 text-white mx-2px'></i>
          </span>
          <div>
            <h4 class='text-dark-tp3'>Something went wrong</h4>
            <span class='text-dark-tp3 text-110'>Lorem ipsum dolor sit amet, consectetur adipiscing elit!</span>
          </div>
        </div>
        <button class='close-btn btn text-grey btn-h-light-danger position-tr mr-1 mt-1'><i class='fa fa-times'></i></button>
        `
    })
  }

  showCenter() {
    this.toasterService.show({
      placement: 'center',

      closeClass: 'btn btn-xs py-0 btn-link bgc-h-white-tp9 no-underline text-white text-150 position-tr mr-n2',

      className: 'bgc-primary-tp2 border-0 px-2 shadow radius-1 overflow-hidden',
      headerClass: 'bg-transparent border-0 text-white text-120 pt-2 ml-5 pl-45',
      bodyClass: 'd-flex text-white pt-0 pb-2',

      header: 'This is a primary notice!',

      body: `        
        <div class="position-tr w-100 h-100 bgc-black-tp5" style="z-index: -1;"></div>
        <div class="mr-3 mt-n4">
          <img class="radius-round p-2px bgc-white" src="assets/image/avatar/avatar.png" />
        </div>
        <div>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. <a role="button" class="text-yellow-l2 font-bolder">Vivamus eget</a> tincidunt velit.
        </div>
        `
    })
  }


  showDark() {
    this.toasterService.show({
      placement: 'tl',

      closeClass: 'btn btn-dark btn-h-danger btn-xs px-2 py-0 text-150 position-tr mr-n25 mt-n3',

      className: 'bgc-black-tp2 radius-2px border-none border-l-4 brc-orange px-3 py-1px shadow',
      headerClass: 'bg-transparent border-0 text-orange-m3 text-115 mt-35',
      bodyClass: 'text-white pt-0 mb-4',

      header: 'This is a dark notice with 5s delay!',

      body: `        
        Lorem ipsum dolor sit amet, consectetur adipiscing elit.
        <a role='button' class='text-warning-m3'>Vivamus eget</a> tincidunt velit.
        <span class='opacity-5'>
          <span class='opacity-5'>
            <i class='fas fa-exclamation-triangle mr-1 mb-1 position-br opacity-5 fa-6x'></i>
          </span>
        </span>
        `
    })
  }


  showSticky() {
    this.toasterService.show({
      placement: 'tc',

      width: 420,

      sticky: true,
      belowNav: true,

      closeClass: 'btn btn-bgc-tp border-0 btn-light-danger btn-xs px-2 py-1px m2-px radius-1 text-100 font-normal position-tr mt-2px mr-2px',

      className: 'brc-success-m1 border-1 border-t-4 radius-0 pr-0',
      headerClass: 'bgc-transparent border-0 text-dark-tp4 pt-2 text-140 mb-3 p-0 pl-3 pr-4',
      titleClass: 'text-400',
      bodyClass: 'pt-0 pl-3 text-105',

      header: 'This is a sticky notice!',

      body: `        
        <div class="bgc-success-d2 px-3 py-3 radius-round ml-1 mr-3 mb-3 float-left">
          <i class="fa fa-check text-140 text-white mx-1px"></i>
        </div>
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. <a role="button" class="text-blue-d2 font-bolder">Vivamus eget</a> tincidunt velit.
        <hr class="my-3" />
        <p class="text-center mt-2 mb-0">
            <button type="button" class="close-btn btn px-4 btn-light-secondary">
              Cancel
            </button>
            <button type="button" class="close-btn btn px-4 btn-green">
              Continue
            </button>
        </p>
        `
    })
  }


  showByTemplate(templateRef: any) {
    this.toasterService.show({
      templateRef: templateRef,

      placement: 'center',
      className: 'bgc-info-tp1',
      progress: 'position-bl bgc-white-tp4 py-2px m-1px',
      progressReverse: true
    })
  }



  ///////////////////////////////////
  removeAll() {
    this.toasterService.removeAll()
  }
}
